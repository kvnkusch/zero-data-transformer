...
async function main() {
  ...
  for (let i = 0; i < NUM_ISSUES; i++) {
    ...
    const created = faker.date.between({from: startDate, to: endDate});
    const modified = faker.date.between({from: created, to: endDate});
  ...
  for (let issueIdx = 0; issueIdx < NUM_ISSUES; issueIdx++) {
    ...
    const issueCreated = faker.date.between({from: startDate, to: endDate});
    ...
    for (let c = 0; c < numComments; c++) {
      ...
      const created = faker.date.between({from: issueCreated, to: endDate});

      await commentWriter.writeRow(
        [commentID, issueID, created.toISOString(), escapeCSV(body), creatorID].join(
          ',',
        ),
      );
    }
