...
export function RelativeTime({timestamp, absolute = false, format}: Props) {
  const now = useNow();
  const timestampMs = timestamp.epochMilliseconds;
  const fullTimestamp = fullTimestampFormat.format(timestampMs);
  return (
    <span title={fullTimestamp}>
      {getRelativeTime(now, timestampMs, absolute, format)}
    </span>
  );
}
...
function useNow(): number {
  ...
  useSharedInterval(() => setNow(Temporal.Now.instant().epochMilliseconds));
  return now;
}
