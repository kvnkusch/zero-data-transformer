...
import {useQuery} from 'zero-data-transformer/react';
...
import {links, type ZbugsHistoryState} from '../../routes.tsx';
...
export function IssuePage({onReady}: {onReady: () => void}) {
  ...
  useLayoutEffect(() => {
    if (!isScrolling) {
      setDisplayed(prev => {
        if (prev === undefined || issue === undefined) {
          return issue;
        }

        const prevViewed = prev.viewState?.viewed;
        const issueViewed = issue.viewState?.viewed;
        const viewedUnchanged =
          (prevViewed === undefined && issueViewed === undefined) ||
          (prevViewed !== undefined &&
            issueViewed !== undefined &&
            prevViewed.equals(issueViewed));

        if (prev.id === issue.id && prev.modified.equals(issue.modified) && viewedUnchanged) {
          return prev;
        }
  ...
  useEffect(() => {
    ...
    const viewed = displayed?.viewState?.viewed;
    if (
      z.userID !== undefined &&
      displayed &&
      (viewed === undefined || Temporal.Instant.compare(displayed.modified, viewed) > 0)
    ) {
